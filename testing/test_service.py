...
CID_URL = 'http://{}/category/predict'.format(URI)
RESULTS_URL = 'http://{}/category/results'.format(URI)

AUTH = HTTPBasicAuth('express','delhivery123')
...
for row in reader:
    ...
    try:
        ...
        r = requests.post(CID_URL, data = json.dumps(payload),
        ...
        while max_retry < 5:
            r2 = requests.post(RESULTS_URL, auth = AUTH, data = json.dumps(cid),
